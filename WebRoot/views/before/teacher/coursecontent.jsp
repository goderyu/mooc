	...
	<div class="wrap">
		...
		<div class="coursecontent-head">
			<ul>
				...
				<li class="selected" onclick="showcatalog();">首页</li>
				<li><form action="/mooc/before/teacher/toteachertask" method="post" style="display:inline-block;">
						<button class="taskbtn">作业</button>
					</form>
				</li>
				<li><form action="/mooc/before/manage/managestudent" method="post" style="display:inline-block;">
						<button class="taskbtn">学生管理</button>
					</form>
				</li>
				<li><a href="/mooc/before/file/tofilemaven" style="color:black;">资料</a></li>
				<li onclick="showdiscuss();">讨论</li>
			</ul>
		</div>
		<div class="catalog">
			<div class="tittle">目录</div>
			<img class="addimg addparent" src="img/course/add.png" /> <input
				type="hidden" id="courseid" value="${courseid}">
			<button class="addparent">添加新章节</button>
			<form action="/mooc/before/editor/coursemsg" style="display:inline-block;">
				<button class="editorcontent">编辑内容</button>
			</form>
				...
				<c:forEach items="${list}" var="firstl" varStatus="fcount">
					<div class="first-level">
						<div class="first-level-text">
							<form action="/mooc/before/course/insertSecondCatalog"
								style="display:inline-block;">
								<input type="hidden" class="firstl"value="${firstl.id}" name="cataloid">
								<input type="hidden" class="courseid" name="courseid"
									value="${courseid}">
								<button class="addchildbtn">
									<img class="addchild" style="width:20px;height:20px;"
										src="img/course/add.png" />
								</button>
							</form>

							<img src="img/course/level1.png"><b>${fcount.count}</b><span
								class="levelname">${firstl.catalogName}</span>
							<button class="editor" state="1" value="${firstl.id}">编辑</button>

							<form style="display:inline-block;"
								action="/mooc/before/course/deleteFirstCatalog"
								onsubmit="return deleteBtn()" method="post">
								<input type="hidden" value="${firstl.id}" name="id"> <input
									type="hidden" class="courseid" name="courseid" value="${courseid}">
								<button class="deletec">删除</button>
							</form>

							<c:if test="${fn:length(list)>1}">
								<c:if test="${!fcount.first}">
									<form style="display:inline-block;" method="post"
										action="/mooc/before/course/upcatalog">
										<input type="hidden" id="courseid" name="courseid"
											value="${courseid}"> <input type="hidden"
											value="${firstl.id}" name="id"> <input type="hidden"
											value="${firstl.cataloorder}" name="cataloorder">
										<button style="border:0;background-color:white;">
											<img class="move up" style="height:20px;"
												src="img/course/up.png">
										</button>
									</form>
								</c:if>
								<c:if test="${!fcount.last}">
									<form style="display:inline-block;" method="post"
										action="/mooc/before/course/downcatalog">
										<input type="hidden" id="courseid" name="courseid"
											value="${courseid}"> <input type="hidden"
											value="${firstl.id}" name="id"> <input type="hidden"
											value="${firstl.cataloorder}" name="cataloorder">
										<button style="border:0;background-color:white;">
											<img class="move down" style="height:20px;"
												src="img/course/down.png">
										</button>
									</form>
								</c:if>
							</c:if>
						...
						<c:forEach items="${list.get(fcount.count-1).courseSecondCatalog}"
							...
							<div class="second-level">
								...
								<button class="editor"  state="2" value="${secondl.id}">编辑</button>
								<form style="display:inline-block;"
									action="/mooc/before/course/deleteSecondCatalog"
									onsubmit="return deleteBtn()" method="post">
									<input type="hidden" value="${secondl.id}" class="secondl" name="id"> <input
										type="hidden" id="courseid" name="courseid"
										value="${courseid}">
									<button class="deletec"  >删除</button>
								</form>
								<c:if
									test="${fn:length(list.get(fcount.count-1).courseSecondCatalog)>1}">
									<c:if test="${!scount.first}">
										<form style="display:inline-block;" method="post"
											action="/mooc/before/course/upscatalog">
											<input type="hidden" name="courseid"
												value="${courseid}"> <input type="hidden"
												value="${secondl.cataloid}" name="cataloid"> <input
												type="hidden" value="${secondl.id}" name="id"> <input
												type="hidden" value="${secondl.scataloorder}"
												name="scataloorder">
											<button style="border:0;background-color:white;">
												<img class="move up" style="height:20px;"
													src="img/course/up.png">
									...
									<c:if test="${!scount.last}">
										<form style="display:inline-block;" method="post"
											action="/mooc/before/course/downscatalog">
											<input type="hidden"  name="courseid"
												value="${courseid}"> <input type="hidden"
												value="${secondl.cataloid}" name="cataloid"> <input
												type="hidden" value="${secondl.id}" name="id"> <input
			...
			<div class="editor-window">
				<div class="editor-window-close">×</div>
				<span class="editor-head">修改章节名称</span>
				<div style="border-bottom:1px solid gray;"></div>
				<div class="editor-mes">
						<form style="display:inline-block;" action="/mooc/before/course/updateCatalog" method="post"> 
						<span class="editor-name">章节名称：</span>
						<input type="text"  name="name" class="editorinput" />
						<input type="hidden" name="id" id="editorid" >
						<input type="hidden"  name="courseid" 
						value="${courseid}">
						<input type="hidden" name="state" id="estate">
						 <input type="submit" class="editorbtn"
						style="border:0;background-color:#55b929;margin-left:100px;margin-top:30px;"
						value="确认" /> 
						</form>
						<input type="submit" class="closebtn" value="取消">
				</div>
			</div>
		</div>
	</div>
	<form action="/mooc/before/course/coursecontent" method="post" id="tzform">
