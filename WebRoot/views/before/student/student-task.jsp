				...
				<tr>
					...
					<c:choose>
						<c:when test="${l.taskComplete==null}">
							...
							<td>未完成</td>
							<td>
								<form action="/mooc/before/student/studenttest" target="_blank"
									method="post" style="display:inline-block;">
									<input type="hidden" name="taskid" value="${l.id}"> <input
										type="hidden" name="taskname" value="${l.taskname}">
									<button class="btn btn-success add-btn">完成作业</button>
								</form>
							</td>
						</c:when>
						<c:otherwise>
							...
							<td>
								<form action="/mooc/before/student/testresult" target="_blank"
									method="post" style="display:inline-block;">
									<input type="hidden" name="taskid" value="${l.id}"> <input
										type="hidden" name="taskname" value="${l.taskname}">
