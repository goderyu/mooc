...
<html>
	...
	<body class="childrenBody">
		...
		<a onclick="importNewStudent();" class="layui-btn layui-btn-normal" style="margin-left:10px;">导入老师</a>
		<a onclick="addNewStudent();" class="layui-btn layui-btn-normal" style="margin-left:10px;">添加老师</a>
		...
		<div id="addUserOneDiv" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background-color:rgba(0,0,0,0.5);">
			<div style="width:400px;margin:100px auto;background-color:white;padding:20px;">
				<form id="excelform" action="/mooc/back/uploadTeachers/uploaduser" method="post" enctype="multipart/form-data">
				...
				<p>2、请按照该
					<a style="color:blue;" href="excel/teacher.xls">模板excel表</a>格式</p><br>
		...
		<div id="insertUserOneDiv" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background-color:rgba(0,0,0,0.5);">
			<div style="width:400px;margin:100px auto;background-color:white;padding:20px;">
				<form action="/mooc/back/teacherManager/addOneUser" method="post">
					<span style="font-size:18px;">添加老师</span>
					<br>
					<br>
					...
					<div class="layui-input-block">
						<input required="required" type="text" id="telephone" name="telephone" lay-verify="title" autocomplete="off" placeholder="请输入手机号" class="layui-input">
						<span id="phone"></span>
					</div>
					<br>
		...
		<div id="updateUserOneDiv" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background-color:rgba(0,0,0,0.5);">
			<div style="width:400px;margin:50px auto;background-color:white;padding:20px;">
				<form id="updateinfo" class="layui-form" action="/mooc/back/teacherManager/updateUser" method="post">
					<span style="font-size:18px;">修改老师信息</span>
					<br>
					<br>
					...
					<div class="layui-input-block">
						<input id="emailinfo" required="required" type="text" name="teacheremail" lay-verify="title" autocomplete="off" placeholder="请输入邮箱" class="layui-input">
						<span id="email"></span>
					</div>
					<br>
					<label class="layui-form-label">性别</label>
					<div class="layui-input-block">
						<select name="teachersex" id="sexselected">
		...
		$("#addbtnuser").click(function(){
			...
			$("#telephone").click(function(){
				if(/^1[3|4|5|7|8][0-9]\d{8}$/.test($("#telephone").val())){
					$.post("/mooc/back/teacherManager/selectTelephone",{
		...
		$("#emailinfo").blur(function(){
			...
			function loadUsers(){
					...
					table.render({
						...
						cols: [
							[ //标题栏
								...
								{ field: 'teacheremail', title: '邮箱' , width: 150}, 
								{ field: 'teachersex', title: '性别' , width: 30  , templet: '#titleTpl'}, 
								{ field: 'poperation', title: '操作', width: '82', fixed: 'right', toolbar: '#barDemo' }
								
							]
						],
						id:'idTest',
						url: '/mooc/back/teacherManager/getAllTeachers?searchVal='+searchval,
						page: {
							groups: 10,
							first: false,
							last: false,
					...
					table.on('tool(demo)', function(obj) {
						...
						if(obj.event === 'del') {
							layer.confirm('确定删除该行？', function(index) {
								window.location.href="/mooc/back/teacherManager/deleteUser?id="+data.id;
								layer.close(index);
							});
						} else if(obj.event === 'edit') {
							tel = data.telephone;
							ema = data.teacheremail;
							$("#updateUserOneDiv").show();
							$("#idinfo").val(data.id);
							$("#nameinfo").val(data.userName);
							$("#telinfo").val(data.telephone);
							$("#emailinfo").val(data.teacheremail);
							if(data.teachersex == false){
						...
						active = {
							getCheckData: function() { //获取选中数据
								...
								layer.confirm('确定删除选中行吗？', function(index) {
									window.location.href="/mooc/back/teacherManager/deleteUsers?userList="+idArr;
									layer.close(index);
								}); 
							},
							getAll: function() { //获取选中数目
								window.location.href="/mooc/back/exportTeachers/exportAllUsers";
							}, 
							isAll: function() { //验证是否全选
								...
								window.location.href="/mooc/back/exportTeachers/exportSelectedUsers?idArr="+idArray.toString();
								...
								table.render({
									...
									cols: [
										[ //标题栏
											...
											, { field: 'telephone', title: '手机号', width: 100 },
											{ field: 'teacheremail', title: '邮箱' , width: 150}, 
											{ field: 'teachersex', title: '性别' , width: 30}, 
										]
									],
									data: data[0].data,
									page: {
				...
				function tijiaoExcel() {
					window.location.href="/mooc/back/uploadTeachers/exportuser";
				}
		...
		<script type="text/html" id="titleTpl">
             {{# if((d.teachersex == false)){ }}
				{{d.teachersex = '女'}}
				{{# }else if((d.teachersex == true)){ }}
				{{d.teachersex = '男'}}
			{{# }}}
		</script>
	</body>
